...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("RecruitMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("RecruitMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("RecruitMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("recruit-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running RecruitMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
